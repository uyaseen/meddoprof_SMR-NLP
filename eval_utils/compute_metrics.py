...
def main(gs, pred, doc_list_gs, subtask=["ner", "class", "norm"]):
    ...
    df_sel = pd.merge(pred, gs, how="right", on=relevant_columns)
    is_valid = df_sel.apply(lambda x: x.isnull().any() == False, axis=1)
    df_sel = df_sel.assign(is_valid=is_valid.values)

    # True Positives:
    TP_per_cc = (
        df_sel[df_sel["is_valid"] == True].groupby("clinical_case")["is_valid"].count()
    )
    ...
    # in the GS
    cc_not_predicted = (
        pred.drop_duplicates(subset=["clinical_case"])
        .merge(
            gs.drop_duplicates(subset=["clinical_case"]),
            on="clinical_case",
            how="right",
            indicator=True,
        )
    ...
    if (P + R) == 0:
        F1 = 0
        warnings.warn(
            "Global F1 score automatically set to zero to avoid division by zero"
        )
