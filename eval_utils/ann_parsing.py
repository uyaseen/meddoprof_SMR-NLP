...
def parse_ann(datapath, relevant_labels, with_notes=False):
    ...
    if with_notes == True:
        df = pd.DataFrame(
            info, columns=["filename", "mark", "label", "offset", "span", "code"]
        )
    else:
        df = pd.DataFrame(info, columns=["filename", "mark", "label", "offset", "span"])

    return df


def parse_one_ann(
    info, root, filename, relevant_labels, ignore_related=False, with_notes=False
):
    """
    ...
    for line in f:
        ...
        if len(splitted) < 3:
            warnings.warn(
                "Wrong Brat format. Line with less than 3 tabular splits in {}:\n {}".format(
                    root + filename, line
                )
        ...
        if len(splitted) > 3:
            warnings.warn(
                "Wrong Brat format. Line with more than 3 tabular splits in {}:\n {}".format(
                    root + filename, line
                )
                + "Skipping {} file...".format(root + filename)
            )
        ...
        if (splitted[0][0] == "T") & (";" in " ".join(splitted[1].split(" ")[1:])):
            warnings.warn(
                "Wrong Brat format. Text span with discontinuous annotation in {}:\n {}".format(
                    root + filename, line
                )
                + "Skipping {} file...".format(root + filename)
            )
